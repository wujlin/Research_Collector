...
原文最后一节先回到一个总问题：对于带确定性内部记忆的自回归生成模型，能不能像在随机热力学里那样，定义一条真正可算的不可逆性量，并把它放进统一框架里？
...
原文最后一节真正想强调的，不只是“我们把公式推完了”，而是这套框架改变了对生成模型不可逆性的看法。
...
原文最后一句其实很值得注意，因为作者在这里把问题从“模型序列是否不可逆”进一步推到了“模型内部是否编码了某种世界过程的时间箭头”。
...
如果把原文 `Summary and perspectives` 压成一句最短总结，就是：
