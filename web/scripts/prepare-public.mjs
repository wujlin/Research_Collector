...
/**
 * 构建前准备脚本：
 * 1. 删除 public/digests（如果是符号链接或目录）
 * 2. 从仓库根复制 digests/ 到 public/
 *
 * 符号链接在本地 dev 环境可用，但 Vercel 等静态托管可能不跟随。
 * Digest figures are vendored below digests/<date>/figures, so the copied digest tree is portable.
 */
...
async function main() {
  ...
  if (mode === "copy") {
    ...
    console.log("[prepare-public] done (digests and vendored figures copied).");
  }
}
